...
<%
    request.setCharacterEncoding("UTF-8");
		...
		String user_addr_idx = request.getParameter("addr_idx");

    int index = Integer.parseInt(request.getParameter("index"));
    /* if(index == 0){
    	out.println("<script>alert('상품을 먼저 담아주세요'); history.back();</script>");
    } */
...
<%
	...
	try{
		...
		String sql = "SELECT last_name,first_name,post_num,addr,detail_addr,phone,user_addr_idx FROM user_addr WHERE idx=? AND ROWNUM=1";
...
<body>
    <div class="row">
        <div class="col-7 my_right">
            <img src="./images/b_bottle.png" alt="" width="200px">
            <div class="mt-3" style="font-size: 12px;">
                <span>카트</span> > <span class="fw-bold">정보</span> > <span>배송</span> > <span>지급</span>
            </div>

            <!-- 사용자 정보 -->
            <div class="mt-5">
                <p class="fw-bold" style="font-size: 18px;">계정정보</p>
                <p style="font-size: 14px;"><%=user_id %></p>
            </div>

            <!-- 메인 입력창 -->
            <div class="mt-5">
                <p class="fw-bold" style="font-size: 18px;">배송 주소</p>
<%--                 		<form action="./info2.jsp" method="get">
		                		<div class="row">
		                			<div class="col">
			                				<select class="form-select mb-3" name="addr_idx" style="height: 50px;">
					                        <option value="0" selected>새주소</option>
					                        <% while(rs.next()){ %>
					                        	<option value="<%=rs.getInt("user_addr_idx")%>"><%=rs.getString("addr")%>,<%=rs.getString("detail_addr")%></option>
					                        <% } %>
					                    </select>
		                			</div>
		                			<div class="col-2" style="padding: 0 12px 0 0;">
	                    			<input type="submit" class="btn btn-primary mb-3" style="height: 50px; width: 115px; font-size: 13px;" value="선택">
	                    		</div>
		                		</div>
                    </form> --%>
                 <form action="info2.jsp?idx=<%=user_idx %>" method="post">
                 	 <% if(!hasD){ %>
                 	 <div class="row mb-3 form-floating">
                        <div class="col">
                            <input type="text" class="form-control" id="floatingName" placeholder="성" name="lastname" style="height: 50px;" value="">
                        </div>
                        <div class="col" style="padding: 0 12px 0 0;">
                            <input type="text" class="form-control" id="exampleInputEmail1" placeholder="이름" name="firstname" style="height: 50px;" value="">
                        </div>
                    </div>
                    <div class="row mb-0">
                    		<div class="col">
                    			<input type="text" class="form-control" id="sample6_postcode" placeholder="우편번호" name="postnum" style="height: 50px;" value="">
                    		</div>
                    		<div class="col-2" style="padding: 0 12px 0 0;">
                    			<input type="button" class="btn btn-primary mb-3" style="height: 50px; width: 115px; font-size: 13px;" onclick="sample6_execDaumPostcode()" value="우편번호 찾기">
                    		</div>
                    </div>
                    <div class="mb-3">
                        <input type="text" class="form-control" id="sample6_address" placeholder="주소" name="addr" style="height: 50px;" value="">
                    </div>
                    <div class="mb-3">
                        <input type="text" class="form-control" id="sample6_detailAddress" placeholder="상세주소" name="detailAddr" style="height: 50px;" value="">
                    </div>
                    <div class="mb-3">
                        <input type="text" class="form-control" id="sample6_extraAddress" placeholder="참고항목" name="extraAddr" style="height: 50px;" value="">
                    </div>
                    <div class="mb-3">
                        <input type="text" class="form-control" id="exampleInputPassword1" placeholder="휴대폰 번호" name="phone" style="height: 50px;" value="">
                    </div>
                 	 <% }else{ %>
	                 <% while(rs.next()){ %>
	                 <div class="row mb-3 form-floating">
                        <div class="col">
                            <input type="text" class="form-control" id="floatingName" placeholder="성" name="lastname" style="height: 50px;" value="<%=rs.getString("last_name")%>">
                        </div>
                        <div class="col" style="padding: 0 12px 0 0;">
                            <input type="text" class="form-control" id="exampleInputEmail1" placeholder="이름" name="firstname" style="height: 50px;" value="<%=rs.getString("first_name")%>">
                        </div>
                    </div>
                    <div class="row mb-0">
                    		<div class="col">
                    			<input type="text" class="form-control" id="sample6_postcode" placeholder="우편번호" name="postnum" style="height: 50px;" value="<%=rs.getInt("post_num")%>">
                    		</div>
                    		<div class="col-2" style="padding: 0 12px 0 0;">
                    			<input type="button" class="btn btn-primary mb-3" style="height: 50px; width: 115px; font-size: 13px;" onclick="sample6_execDaumPostcode()" value="우편번호 찾기">
                    		</div>
                    ...
                    <div class="mb-3">
                        <input type="text" class="form-control" id="sample6_address" placeholder="주소" name="addr" style="height: 50px;" value="<%=rs.getString("addr")%>">
                    </div>
                    <div class="mb-3">
                        <input type="text" class="form-control" id="sample6_detailAddress" placeholder="상세주소" name="detailAddr" style="height: 50px;" value="<%=rs.getString("detail_addr")%>">
                    </div>
                    <div class="mb-3">
                        <input type="text" class="form-control" id="sample6_extraAddress" placeholder="참고항목" name="extraAddr" style="height: 50px;" value="">
                    </div>
                    <div class="mb-3">
                        <input type="text" class="form-control" id="exampleInputPassword1" placeholder="휴대폰 번호" name="phone" style="height: 50px;" value="<%=rs.getString("phone")%>">
                    </div>
	                 <% }} %>
                    ...
                    <div class="d-flex mt-4 mb-4">
                        <span class="mt-4"><a href="#" style="text-decoration: none;">< 장바구니로 돌아가기</a></span>
                        ...
                        <% for(int i=1; i<index; i++){ %>
		                    ...
		                    <input type="hidden" id="total-price" name="total-price" value="<%=total_price%>">
		                    <input type="hidden" name="index" value="<%=index%>">
		                    
                        <input type="submit" class="btn btn-primary ms-auto" style="height: 60px; width: 150px;" value="배송 계속하기">
                    </div>
        ...
        <div class="col my_left" style="padding-left: 3%">
            ...
            <div class="mt-2 fw-bold" style="font-size: 14px; color: rgb(102, 102, 102);">
                <span>세금 포함</span>
